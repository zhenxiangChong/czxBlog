...
export function useHitokoto() {
  ...
  const cache = useSessionStorage<{ data?: HitokotoData; updatedAt?: number }>(
    "__VUEPRESS_LANDING_HITOKOTO__",
    {}
  );
  ...
  async function fetchHitokoto({
    ...
    if (
      ...
      Date.now() - cache.value.updatedAt < 6000 &&
      cache.value.data
    ) {
  ...
  onMounted(() => {
    updateHitokoto();
    interval = setInterval(updateHitokoto, 12000);
  });
